...
function LoginForm() {
  ...
  useEffect(() => {
    ...
    if (success === "signup") {
      setSuccessMessage(t("auth.login.successAfterSignup"));
      return;
    }
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          typeof data.message === "string" ? data.message : t("auth.login.errorGeneric"),
        );
      ...
      setError(err instanceof Error ? err.message : t("auth.login.errorUnknown"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        <label htmlFor="login-email" className={labelCls}>{t("auth.login.emailLabel")}</label>
        <input
          id="login-email"
          type="email"
          required
    ...
    <div className="mx-auto max-w-md space-y-6">
      ...
      >
        <Link href="/" className="hover:text-primary">
          {t("auth.login.breadcrumbHome")}
        </Link>
        <span aria-hidden="true" className="text-foreground/25">/</span>
        <span className="font-semibold text-foreground">{t("auth.login.breadcrumbCurrent")}</span>
      </nav>
