  ...
  return (
    <div className="space-y-6">
      <div className="card p-6 space-y-3">
        <h1 className="text-2xl font-semibold text-foreground">Recherche produits</h1>
        <form className="flex flex-col gap-3 md:flex-row">
          <input
            name="q"
            defaultValue={query}
            placeholder="Ex: Scanner, monitoring..."
            className="flex-1 rounded-md border border-primary/25 bg-background px-3 py-2 text-sm text-foreground shadow-sm focus:border-primary focus:outline-none"
          />
        ...
        {results.map((product) => (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-foreground/60">{product.sku}</span>
            </div>
            <p className="text-sm text-primary">
              {(product.priceCents / 100).toFixed(2)} {product.currency}
            </p>
            <p className="text-xs text-foreground/70 line-clamp-2">{product.description}</p>
          </Link>
        ))}
      </div>
    </div>
