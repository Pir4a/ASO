...
export function BuyNowButton({ productId, productName, disabled }: BuyNowButtonProps) {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      addToast(t("cart.buyNowToast").replace("{name}", productName), "success");
      router.push("/checkout");
    } catch (e: unknown) {
      addToast(e instanceof Error ? e.message : t("cart.buyNowError"), "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    ...
    >
      {loading ? t("cart.buyNowRedirecting") : t("cart.buyNow")}
    </button>
  );
}
