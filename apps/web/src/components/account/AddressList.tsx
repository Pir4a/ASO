...
export function AddressList() {
    ...
    const handleCreate = async (data: AddressFormData) => {
        ...
        if (!res.ok) {
            ...
            const msg = typeof body.message === "string" ? body.message : t("profile.addresses.createError");
            flashAndClear("error", msg);
            throw new Error(msg);
        }
        setCreating(false);
        await fetchAddresses();
        flashAndClear("success", t("profile.addresses.created"));
    };
    ...
    const handleUpdate = async (data: AddressFormData) => {
        ...
        if (!res.ok) {
            const body = (await res.json().catch(() => ({}))) as Record<string, unknown>;
            const msg = typeof body.message === "string" ? body.message : t("profile.addresses.updateError");
            flashAndClear("error", msg);
            throw new Error(msg);
        }
        setEditing(null);
        await fetchAddresses();
        flashAndClear("success", t("profile.addresses.updated"));
    };

    const handleDelete = async (id: string) => {
        if (!confirm(t("profile.addresses.deleteConfirm"))) return;
        const res = await authFetch(`/profile/addresses/${id}`, { method: "DELETE" });
        if (res.ok) {
            await fetchAddresses();
            flashAndClear("success", t("profile.addresses.deleted"));
        } else {
            flashAndClear("error", t("profile.addresses.deleteError"));
        }
    };
