...
export function PaymentMethodList() {
    ...
    const fetchMethods = useCallback(async () => {
        try {
            ...
            if (res.ok) {
                ...
                flashAndClear("error", t("payment.stripeMisconfigured"));
                setMethods([]);
            } else {
                setMethods([]);
            }
    ...
    const handleDelete = async (id: string) => {
        if (!confirm(t("payment.confirmDelete"))) return;
        setBusy(id);
        try {
            const res = await authFetch(`/payment/methods/${id}`, { method: "DELETE" });
            if (!res.ok) throw new Error(t("payment.deleteFailed"));
            await fetchMethods();
            flashAndClear("success", t("payment.deleted"));
        } catch (e) {
            flashAndClear(
                "error",
                e instanceof Error ? e.message : t("payment.deleteFailed"),
            );
        } finally {
            setBusy(null);
        }
    ...
    const handleSetDefault = async (id: string) => {
        ...
        try {
            ...
            if (!res.ok) throw new Error(t("payment.updateFailed"));
            await fetchMethods();
            flashAndClear("success", t("payment.defaultSet"));
        } catch (e) {
            flashAndClear(
                "error",
                e instanceof Error ? e.message : t("payment.updateFailed"),
            );
        } finally {
            setBusy(null);
        }
            ...
            {loading ? (
                ...
                <ul className="space-y-3" role="list">
                    {methods.map((pm) => {
                        ...
                        const cardName = brandLabel(pm.brand, t("payment.cardLabel"));
                        return (
                            <li
                                key={pm.id}
                            ...
                            >
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    <p className="mt-0.5 text-[12px] text-foreground/55">
                                        {t("payment.expires")} {formatExpiry(pm.expMonth, pm.expYear)}
                                    </p>
                                </div>
                                {isDefault && (
                                ...
                                <div className="flex flex-wrap items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        disabled={isBusy}
                                        aria-label={t("payment.deleteAria")}
