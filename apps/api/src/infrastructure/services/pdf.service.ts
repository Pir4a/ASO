  ...
  generateInvoice(order: Order): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      doc
        ...
        .fillColor('#ffffff')
        .font('Helvetica-Bold')
        .text('FACTURE', pageW - margin - 80, 38, {
          width: 80,
          align: 'right',
          lineBreak: false,
        });
      doc
        ...
        .fontSize(9)
        .fillColor('#b3eef2')
        .text(orderNumber, pageW - margin - 200, 56, {
          width: 200,
          align: 'right',
          lineBreak: false,
        });
      ...
      doc
        ...
        .fontSize(8.5)
        .text(
          'SIRET 802 145 678 00012 · TVA FR12 802145678',
          margin,
          metaTop + 72,
        );
      ...
      doc
        .roundedRect(metaX, metaTop - 4, metaW, 96, 6)
        .fillColor(PANEL)
        .fill();
      doc
        .fillColor(INK_MUTED)
      ...
      const addr = (order.billingAddress ||
        order.shippingAddress) as unknown as
      ...
      if (addr) {
        ...
        let y = billY + 30;
        if (addr.street) {
          doc.text(String(addr.street), margin, y);
          y += 13;
        }
        if (addr.address2) {
          doc.text(String(addr.address2), margin, y);
          y += 13;
        }
        ...
        if (addr.country) {
          doc.text(String(addr.country), margin, y);
          y += 13;
        }
        if (addr.phone) {
          doc
            .fillColor(INK_MUTED)
            .fontSize(9)
            .text(String(addr.phone), margin, y);
        }
      } else {
        doc
          .fillColor(INK_MUTED)
          .fontSize(9.5)
          .text('Adresse non renseignée.', margin, billY + 14);
      }
      ...
      doc.text('PRIX UNITAIRE', colUnit, tableTop + 7, {
        width: 80,
        align: 'right',
      });
      doc.text('TOTAL HT', colLine, tableTop + 7, {
        width: 80,
        align: 'right',
      });
      doc.restore();
      ...
      (order.items || []).forEach((item, i) => {
        ...
        doc
          ...
          .font('Helvetica-Bold')
          .fontSize(9.5)
          .text(item.productName, colProd + 8, y, {
            width: colQty - colProd - 20,
          });
        if (item.productSku) {
          doc
            .fillColor(INK_MUTED)
            .font('Helvetica')
            .fontSize(8.5)
            .text(`Réf. ${item.productSku}`, colProd + 8, y + 11, {
              width: colQty - colProd - 20,
            });
        }
        doc.fillColor(INK).font('Helvetica').fontSize(10);
        doc.text(String(item.quantity), colQty, y + 4, {
          width: 60,
          align: 'right',
        });
        doc.text(
          this.formatPrice(Number(item.price), item.currency),
          colUnit,
          y + 4,
        ...
        doc
          .font('Helvetica-Bold')
          .fillColor(INK)
          .text(this.formatPrice(lineTotal, item.currency), colLine, y + 4, {
            width: 80,
            align: 'right',
          });
        y += rowH;
      ...
      const totalRow = (
        ...
        doc
          .fillColor(opts.emphasis ? NAVY : INK)
          .font('Helvetica-Bold')
      ...
      totalRow(
        ...
        this.formatPrice(totalVat, order.currency),
      );

      // Divider before total
      doc
        .moveTo(totalsX, y - 4)
        .lineTo(totalsX + totalsW, y - 4)
        .strokeColor(LINE)
        .stroke();
      y += 4;
      ...
      doc
        ...
        .fontSize(8)
        .text(
          'Merci pour votre commande chez Althea Systems.',
          margin,
          footerY + 10,
          {
            width: contentW,
        ...
        .text(
          'Document généré électroniquement, sans signature requise. Conservez-le pour vos archives comptables.',
          margin,
          footerY + 22,
          {
            width: contentW,
        ...
        .fontSize(8.5)
        .font('Helvetica-Bold')
        .text('althea.fr', margin, footerY + 38, {
          width: contentW,
          align: 'center',
        });
