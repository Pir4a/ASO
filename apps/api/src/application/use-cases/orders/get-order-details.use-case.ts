...
export class GetOrderDetailsUseCase {
  ...
  async execute(
    ...
    let order = await this.orderRepository.findOneByIdAndUserId(
      orderId,
      userId,
    );
    ...
    if (order.paymentMethodId && (!order.paymentBrand || !order.paymentLast4)) {
      try {
        ...
        if (pm?.brand || pm?.last4) {
          ...
          order =
            (await this.orderRepository.findOneByIdAndUserId(
              orderId,
              userId,
            )) ?? order;
        }
